...
const firebaseConfig = {
  ...
  //firebaseConfig : firebase 프로젝트 설정 값 객체 apo키, 도메인 인증, 데이터베이스 키값

const app = initializeApp(firebaseConfig); //firebaseconfig를 기반으로 firebase설정 값을 초기화
const auth = getAuth(app);// 초기화된 앱을 기반으로 firebase인증 객체 생성
const provider = new GoogleAuthProvider(); // google로그인 기능을 사용할때 추가하는 프로바이더 객체 생성
const database = getDatabase() // 초기화된 앱을 기반으로 firebase 데이터베이스 객체생성
...
export async function googleLogin(){
  try{
    ...
    // signInWithPopup :  firebase 자체에 있는 인증 객체 provider를 인자를 받아서 google계정을 연동해서 로그인할 수 있게 하는 매서드
...
export async function uploadImgs(file) {
  ...
  try{
    const id = uuid();
    const imgRef = storageRef(storage, `imgs/${id}`)
    console.log(imgRef)
...
/*
모든 데이터를 클라이언트로 전송하는 로직이기 때문에 클라이언트 자체의 메모리를 사용하기 때문에 로딩이나 메모리 처리의
과부하의 문제가 생김
...
*/  
//  export async function getCategoryProduct(catagory){
//     try{
//       return get(databaseRef(database,'products')) .then((snapshot)=>{
//         if(snapshot.exists()){
//           const allProduct = Object.values(snapshot.val());
//           const filterProduct = allProduct.filter((product)=>product.catagory === catagory);
