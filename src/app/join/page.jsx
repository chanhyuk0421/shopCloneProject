...
export default function JoinPage(){
    ...
    const validatorName = (userName) =>{
        if(!userName){
            setNameError('이름을 입력 해 주세요');
            return false
        }
        if(userName.length <= 2 || userName.lenth > 10){
            setNameError('이름을 2글자 이상 10글자 이하로 작성 해 주세요')
            return false
        }
        ...
        /* 
            정규표현식
            /^[A-Za-z가-힣\s'-]  입력한 이름이 인터넷 대소문자, 한글, 공백, 작은 따옴표, 하이픈 만으로
