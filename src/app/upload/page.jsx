...
export default function UploadPage(){
    ...
    이미지는 db와 저장 할 수 없다.
    이미지를 저장할 수 있는 클라우드(firebase strorage, cloudinary)를 사용해서 이미지를 따로 저장한 후에 이미지 경로만 받아서
    ...
    */ 
    ...
    const ProductInfoChange = (e) =>{
        ...
        /*
        name을 []로 묶는 이유는 계산된 속성명을 사용하기 위해서 사용
        변수의 값을 객체의 키로 동적으로 설정 할 수 있게 됨
        입력 폼에서 여러개의 입력창을 수정해야 하기 때문에 하나의 입력창을 수정할 때 마다 그 선택자를
        동적으로 받아와서 값을 받아옴
        []로 묶지 않으면 하나의 값으로만 인식해서 입력값을 제대로 받아오지 못하게 됨

        []x = 변수 이름이 아니라 문자열의 키 값으로 사용
        [] =  변구의 값이 동적으로 계산되어 사용 
        */ 
    }
    ...
    const uploadSubmit = async(e) =>{
        ...
        try{
            ...
            setIsLoading(false);
            // finally는 try와 catch와 관계없이 try와 catch가 실행되고 무조건적으로 실행되는 블록
            //  
        }
        ...
        <UploadContainer>
            ...
            <form onSubmit={uploadSubmit}>
                ...
                <input 
                    type="text"
                    name="option"
                    placeholder="상품 옵션을 ,로 구분해서 올려주세요."
