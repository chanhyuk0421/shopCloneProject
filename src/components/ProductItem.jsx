...
import { useRouter } from "next/navigation"; //router 관련 기능은 next/router에서 next/navigation으로 변경됨
...
export default function ProductItem({product}){
    ...
    const detailNavigate = ()=>{
        ...
        push()  // url 이동/ 히스토리 저장(뒤로가기 버튼시 이전 url로  이동가능)
        replace()  // url이동 / 히스토리에 저장하지 않음(뒤로가기 동작시 이전 url로 가지 않음)
        go() // 숫자(히스토리스택)만큼 뒤로가거나 앞으로 가기가 가능
        ex) .go(2) 2단계 앞으로 이동 .go(-2) 2단계 뒤로 ㅣㅇ동
        ...
        <DetailItem onClick={detailNavigate}>
            ...
            <TextWrap>
                ...
                <div className="itemFlex">
                    <p className="itemPrice">{formarCurrency(product.price)}원</p>
